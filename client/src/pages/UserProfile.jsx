...
const UserProfile = () => {
  ...
  useEffect(() => {
    const getUser = async () => {
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/users/${currentUser.id}`,
        ...
        console.log("Avatar recibido del backend:", avatar);
        setName(name);
        setEmail(email);
        setAvatar(avatar);
    }
  ...
  const changeAvatarHandler = async () => {
        ...
        try {
          ...
          console.log("Archivo avatar antes de enviar:", avatar);
          const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/users/change-avatar`,
            ...
            console.log("Respuesta después de actualizar avatar:", response); // Verificar respuesta completa
            setAvatar(response?.data.avatar);
            console.log("Nuevo avatar asignado al estado:", response.data.avatar);
        } catch (error) {
          console.log(error);
  ...
  return (
    <section className='profile'>
      <div className="container profile__container">
        <Link to={`/myposts/${currentUser.id}`} className='btn'>Mis Posteos</Link>

        <div className="profile__details">
          <div className="avatar__wrapper">
            <div className="profile__avatar">
            <img src={`${process.env.REACT_APP_BASE_URL}/uploads/${avatar}`} alt="Avatar" key={avatar} />
